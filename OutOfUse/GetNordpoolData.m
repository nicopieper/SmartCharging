% https://www.nordpoolgroup.com/api/marketdata/page/194?currency=,EUR,EUR,EUR&Time.End=09-07-2020&entityName=50HZ

TimeInterval=strcat(datestr(Time.Start, 'yyyymmdd'), '0000_', datestr(Time.End, 'yyyymmdd'), '23');
...
if isfile(StorageFile) && ProcessDataNewIntraday==0
    ...
    NordpoolURL='https://www.nordpoolgroup.com/api/marketdata/page/194?currency=,EUR,EUR,EUR&Time.End=';
    Areas=["50HZ"; "AMP"];
    IntradayRealH=zeros((days(Time.End-Time.Start)+1)*24,length(Areas));
    
    for i=1:length(Areas)    
        k=1;
        for Date=Time.Start:Time.End
