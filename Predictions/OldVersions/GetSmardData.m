...
if ~isfile(StorageFile) || ProcessDataNewSmard==1
    ...
    TimeH=datetime(strcat(PricesRealH(:,1), " ", PricesRealH(:,2)),'InputFormat','dd.MM.yyyy HH:mm', 'TimeZone', 'Africa/Tunis'); % Use Tunesian TimeZones, as it does not consider DST.
    ...
    TimeH=DeleteDST(TimeH, DSTChangesH, 1);
    ...
    TimeQH=datetime(strcat(GenRealQH(:,1), " ", GenRealQH(:,2)),'InputFormat','dd.MM.yyyy HH:mm', 'TimeZone', 'Africa/Tunis');
...
%     NetExportH=DeleteDST(FillMissingValues(str2double(strrep(NetExportH, ',', '.')), 1), DSTChangesH, 1);    
    ...
    %% Store Data in File
    save(StorageFile, 'GenPredFREH', 'GenPredFREQH', 'GenPredH', 'GenPredQH', 'GenRealFREH', 'GenRealFREQH', 'GenRealH', 'GenRealQH', 'LoadPredH', 'LoadPredQH', 'LoadRealH', 'LoadRealQH', 'PricesRealH', 'TimeH', 'TimeQH', "-v7.3")
