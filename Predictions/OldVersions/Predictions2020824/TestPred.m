function [Prediction, PredictionMat, TargetMat, MAEConst, mMAPEConst, RMSEConst] = TestPred(PredMethod, PredictorMat, TargetDelayed, Target, TimeVecPred, TimeStepPredInd, RangeTestPredInd, MaxDelayInd, ForecastIntervalInd, Demo, TargetName, ActivateWaitbar)
%% Description
...
PredictionMat=zeros(ForecastIntervalInd, floor((RangeTestPredInd(2)-RangeTestPredInd(1))/ForecastIntervalInd), NumPredMethod);
TargetMat=zeros(ForecastIntervalInd, floor((RangeTestPredInd(2)-RangeTestPredInd(1))/ForecastIntervalInd));
...
if Demo
    ...
    for p=1:NumPredMethod % Create one Figure Property for each model
        figPred{p}=plot(TimeVecPred(max(TimeInd-ForecastIntervalInd+ForecastDuration, RangeTestPredInd(1)):TimeInd+ForecastDuration), Prediction(max(TimeInd-ForecastIntervalInd+ForecastDuration, RangeTestPredInd(1)):TimeInd+ForecastDuration,p), 'Color', PlotColors(p,:));        
    end       
...
while TimeInd<=RangeTestPredInd(2)
    for ForecastDuration=0:min(ForecastIntervalInd-1, RangeTestPredInd(2)-TimeInd)        
        for p=1:NumPredMethod
            ...
            if Demo  
                ...
                title(strcat(TargetName, " Prediction vs. Target at ", datestr(TimeVecPred(TimeInd),'dd.mm.yyyy HH:MM')),'Interpreter','none')
                xlim([TimeVecPred(TimeInd-36*TimeStepPredInd+max(0,-ForecastIntervalInd+24*TimeStepPredInd+ForecastDuration+1)) TimeVecPred(EndCounter+3)]) % Create a moving plot 
                ylim([ymin ymax])        
...
hold on
for p=1:NumPredMethod
    plot(TimeVecPred(RangeTestPredInd(1):RangeTestPredInd(1)+ForecastIntervalInd-1),MAE(:,p), 'Color', PlotColors(p,:))
end    
xtickformat('HH:mm')
title(strcat("Mean Absolute Error predicting 1 to ", num2str(round(ForecastIntervalInd/TimeStepPredInd)), " hours of ", TargetName),'Interpreter','none')
