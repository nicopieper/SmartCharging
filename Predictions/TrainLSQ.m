function [LSQCoeffs, TrainFun] = TrainLSQ(Target, TargetDelayed, PredictorMat, ForecastIntervalPredInd, RangeTrain, MaxDelayInd)
%% Description
% Tbd
...
for ForecastDuration=0:ForecastIntervalPredInd-1 % For each hour of ForecastInterval one prediction model is trained. That means there is one model for a one hour prediction, one for a two hour prediction etc.
    ...
    LSQCoeffs(ForecastDuration+1,:) = lsqcurvefit(TrainFun,LSQCoeffs(end,:),PredictorMatInput,Target(RangeTrain(1)+MaxDelayInd+ForecastDuration:1:RangeTrain(2),:), [], [], opts); % If ForecastDuration==0, then the model for a 1h prediction is trained
