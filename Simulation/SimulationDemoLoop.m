...
%     addpoints(figSpotmarketPred{p}, Time.VecDateNum(TimeInd-400:TimeInd),SpotmarketPred(TimeInd-400:TimeInd))
%     clearpoints(figPVPred{p})
%     addpoints(figPVPred{p}, Time.VecDateNum(TimeInd-400:TimeInd),PVPredQH(TimeInd-400:TimeInd))
    
    ForcastLength=min(ForecastIntervalInd-1, Range.TestInd(2)-TimeInd);
    for ForecastDuration=0:Time.StepIndDemo:min(ForecastIntervalInd-1, Range.TestInd(2)-TimeInd)
        for p=1:NumPredMethod
%             figure(11)


            subplot(2,2,1)
            EndCounter=max(EndCounter,TimeInd+ForecastDuration+Time.StepIndDemo-1);
%             SpotmarketPred(TimeInd+ForecastDuration:TimeInd+ForecastDuration+Time.StepIndDemo-1)=SpotmarketPredMat(ForecastDuration+1:ForecastDuration+Time.StepIndDemo,round(days(Time.Vec(TimeInd)-DemoStartDay)+1));
            SpotmarketPred(TimeInd+ForecastDuration:TimeInd+ForecastDuration+Time.StepIndDemo-1)=SpotmarketPredMat(ForecastDuration+1:ForecastDuration+Time.StepIndDemo,TimeInd);
            if ForecastDuration<=ForcastLength-24*Time.StepInd
                clearpoints(figSpotmarketPred{p})
                addpoints(figSpotmarketPred{p},Time.VecDateNum(max([DemoStart, TimeInd-300]):EndCounter),SpotmarketPred(max([DemoStart, TimeInd-300]):EndCounter))
%                 b{end+1,1}=SpotmarketPred(max([DemoStart, TimeInd-300]):EndCounter);
            else
                addpoints(figSpotmarketPred{p},Time.VecDateNum(TimeInd+ForecastDuration:TimeInd+ForecastDuration+Time.StepIndDemo-1),SpotmarketPred(TimeInd+ForecastDuration:TimeInd+ForecastDuration+Time.StepIndDemo-1))
            end
            %title(strcat(SpotmarketLabel, " ", datestr(Time.Vec(TimeInd),'dd.mm.yyyy HH:MM')),'Interpreter','none')
            xlim([Time.VecDateNum(TimeInd-36*Time.StepInd+max(0,-ForecastIntervalInd+24*Time.StepInd+ForecastDuration+Time.StepIndDemo)) Time.VecDateNum(EndCounter+3)]) % Create a moving plot 
            ...
            if ShowPVPred
                subplot(2,2,4)
%                 PVPredQH(TimeInd+ForecastDuration:TimeInd+ForecastDuration+Time.StepIndDemo-1)=PVPredMat(ForecastDuration+1:ForecastDuration+Time.StepIndDemo,round(days(Time.Vec(TimeInd)-RangeTestDate(1))+1));
                PVPredQH(TimeInd+ForecastDuration:TimeInd+ForecastDuration+Time.StepIndDemo-1)=PVPredMat(ForecastDuration+1:ForecastDuration+Time.StepIndDemo,TimeInd);
                if ForecastDuration<=ForcastLength-24*Time.StepInd
                    clearpoints(figPVPred{p})
                    addpoints(figPVPred{p},Time.VecDateNum(max([DemoStart, TimeInd-300]):EndCounter),PVPredQH(max([DemoStart, TimeInd-300]):EndCounter))
                else
                    addpoints(figPVPred{p},Time.VecDateNum(TimeInd+ForecastDuration:TimeInd+ForecastDuration+Time.StepIndDemo-1),PVPredQH(TimeInd+ForecastDuration:TimeInd+ForecastDuration+Time.StepIndDemo-1))
                end
    %                 title(strcat(SpotmarketLabel, " ", datestr(Time.Vec(TimeInd),'dd.mm.yyyy HH:MM')),'Interpreter','none')
                xlim([Time.VecDateNum(TimeInd-36*Time.StepInd+max(0,-ForecastIntervalInd+24*Time.StepInd+ForecastDuration+Time.StepIndDemo)) Time.VecDateNum(EndCounter+3)]) % Create a moving plot 
            end
...
%             %title(strcat(SpotmarketLabel, " ", datestr(Time.Vec(TimeInd),'dd.mm.yyyy HH:MM')),'Interpreter','none')
%             xlim([Time.Vec(TimeInd-36*Time.StepInd+max(0,-ForecastIntervalInd+24*Time.StepInd+ForecastDuration+1)) Time.Vec(EndCounter+3)]) % Create a moving plot 
%             ylim([yminSpotmarket ymaxSpotmarket])
% 
%             subplot(2,2,2)
%             xlim([Time.Vec(TimeInd-36*Time.StepInd+max(0,-ForecastIntervalInd+24*Time.StepInd+ForecastDuration+1)) Time.Vec(EndCounter+3)]) % Create a moving plot 
...
subplot(2,2,1)
addpoints(figSpotmarketReal,Time.VecDateNum(TimeInd:TimeInd+Time.StepIndDemo-1),SpotmarketReal(TimeInd:TimeInd+Time.StepIndDemo-1))
ylim([yminSpotmarket ymaxSpotmarket])
% title(strcat(SpotmarketLabel, " ", datestr(Time.Vec(TimeInd),'dd.mm.yyyy HH:MM')),'Interpreter','none')

subplot(2,2,2)
addpoints(figResPoDemRealNeg,Time.VecDateNum(TimeInd:TimeInd+Time.StepIndDemo-1),ResPoDemRealQH(TimeInd:TimeInd+Time.StepIndDemo-1,1))
addpoints(figResPoDemRealPos,Time.VecDateNum(TimeInd:TimeInd+Time.StepIndDemo-1),ResPoDemRealQH(TimeInd:TimeInd+Time.StepIndDemo-1,2))
% title(strcat(ResPoDemLabel, " ", datestr(Time.Vec(TimeInd),'dd.mm.yyyy HH:MM')),'Interpreter','none')
ylim([yminResPoDem ymaxResPoDem])

subplot(2,2,3)
addpoints(figSoCPlot,Time.VecDateNum(TimeInd:TimeInd+Time.StepIndDemo-1),double(Users{DemoUser}.LogbookBase(TimeInd:TimeInd+Time.StepIndDemo-1,7))/double(Users{DemoUser}.BatterySize))
% title(strcat(SoCPlotLabel, " ", datestr(Time.Vec(TimeInd),'dd.mm.yyyy HH:MM')),'Interpreter','none')

if ShowPVPred
    subplot(2,2,4)
    addpoints(figPVPlot,Time.VecDateNum(TimeInd:TimeInd+Time.StepIndDemo-1),double(PVPlants{Users{DemoUser}.PVPlant}.Profile(TimeInd:TimeInd+Time.StepIndDemo-1)))
