...
ResPoOffers(:,1,PreAlgoCounter+1)=ResPoPricesReal4H(floor((TimeInd+TD.Main)/(4*Time.StepInd))+1-hour(TimeOfPreAlgo1)/4:floor((TimeInd+TD.Main)/(4*Time.StepInd))+1-hour(TimeOfPreAlgo1)/4+5,3)/1000*ResPoPriceFactor; % [€/kW] The offered reserve price for the next day. Get the marginal prices of yesterdays auctions (for the fulfillment today, as the autions were yesterday, the prices are already available) and reduce it by a fixed factor to enhance the probability of a successful offer.
ResPoOfferPrices=[ResPoOffers(hour(TimeOfPreAlgo1)/4+1:6,1,PreAlgoCounter); ResPoOffers(1:6,1,PreAlgoCounter+1)];
