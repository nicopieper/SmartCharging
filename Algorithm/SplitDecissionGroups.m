...
%     DecissionGroups{k,1}=AvailabilityOrder((k-1)*NumUsers/NumDecisionGroups+1:k*NumUsers/NumDecisionGroups);
    ...
    for l=DecissionGroups{k,1}'
        DecissionGroups{k,4}=DecissionGroups{k,4} + Users{l+1}.Logbook(TimeInd+TD.User:4*Time.StepInd/ConstantResPoPowerPeriodsScaling:TimeInd+TD.User+ConsPeriods*length(ResPoBlockedIndices)/ConstantResPoPowerPeriodsScaling*Time.StepInd-1,7);
