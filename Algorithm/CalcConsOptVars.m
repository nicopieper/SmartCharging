...
ResPoOffers(:,1,PreAlgoCounter+1)=repelem(ResPoPricesReal4H(floor((TimeInd+TD.Main)/(4*Time.StepInd))+1-hour(TimeOfPreAlgo(1))/4:floor((TimeInd+TD.Main)/(4*Time.StepInd))+1-hour(TimeOfPreAlgo(1))/4+5,3)/1000*ResPoPriceFactor, ConstantResPoPowerPeriodsScaling); % [€/kW] The offered reserve price for the next day. Get the marginal prices of yesterdays auctions (for the fulfillment today, as the autions were yesterday, the prices are already available) and reduce it by a fixed factor to enhance the probability of a successful offer.
...
ResPoOfferPrices=repelem([ResPoOfferPrices; ResPoOffers(1:ControlPeriods/(4*Time.StepInd/ConstantResPoPowerPeriodsScaling)-length(ResPoOfferPrices),1,PreAlgoCounter+1)], 4*Time.StepInd/ConstantResPoPowerPeriodsScaling); % [€/kW]
...
ResEnOfferPrices=repelem([ResEnOfferPrices; ResEnOffers(1:ControlPeriods/(4*Time.StepInd/ConstantResPoPowerPeriodsScaling)-length(ResEnOfferPrices),1,PreAlgoCounter+1)], 4*Time.StepInd/ConstantResPoPowerPeriodsScaling); % [€/kW]
...
if UseIndividualEEGBonus
    ...
    CostsPV=CostsPV*(Users{1}.EEGBonus/100); % 0.097€
end
